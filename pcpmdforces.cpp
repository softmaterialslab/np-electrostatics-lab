...
void for_cpmd_calculate_force(vector<VERTEX>& s, vector<PARTICLE>& ion, INTERFACE& nanoparticle) 
{
  ...
  #pragma omp parallel default(shared) private(kloop, iloop, l1, i1, j1, gEwq, gwEq, gwEq_EwEq, gwq, gww_wEw_EwEw, hqEw, hqEq, hEqw, hEqEq, hEqEw, h0, h1, h2, h3) num_threads(THREADSIZE)
  {
	...
	h2 = h2 + ( ( (s[l1].gradGion[iloop])^(-1.0) ) ^ ( (aqw * s[l1].w + bqEqw * innerh2[l1]) * s[l1].a ) );
      ...
      h3 = VECTOR3D(0, 0, 0);
      for (l1 = 0; l1 < s.size(); l1++) 
	h3 = h3 + ( GradndotGrad(s[l1].posvec, ion[iloop].posvec, s[l1].normalvec) ^ (innerh4[l1] * s[l1].a) );
  ...
  else //if not POLARIZED, that is no induced charges due to difference in dielectric properties of NP and its environment
  {
    ...
    #pragma omp parallel default(shared) private(iloop, j1, h0, h1) num_threads(THREADSIZE)
    {
      #pragma omp for schedule(dynamic,CHUNKSIZE) nowait
      for (iloop = 0; iloop < ion.size(); iloop++)
      {

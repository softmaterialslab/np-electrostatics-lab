...
void INTERFACE::set_up(double salt_conc_in, double salt_conc_out, double salt_valency_in, double salt_valency_out, int N, double b)
{
...
void INTERFACE::put_saltions_inside(vector<PARTICLE>& saltion_in, int valency, double concentration, double diameter, vector<PARTICLE>& ion)
{
  ...
  unsigned int total_saltions_inside = int(2 * (concentration * 0.6) * (volume_sphere * unitlength * unitlength * unitlength));		// NOTE there is a change in the factor, i think it was wrong before
  ...
  while (saltion_in.size() != total_saltions_inside)
  {
    ...
    z = (1 - z) * (-r0) + z * (r0);
    VECTOR3D posvec = VECTOR3D(x,y,z);
    if (posvec.GetMagnitude() > r0 - diameter)									// putting an extra ion diameter length away from interface
      continue;
    bool continuewhile = false;
    for (unsigned int i = 0; i < ion.size() && continuewhile == false; i++)
    ...
    if (continuewhile == true)
      continue;
    saltion_in.push_back(PARTICLE(int(ion.size())+1,diameter,valency,valency*1.0,1.0,ein,posvec));		// create a salt ion
    ion.push_back(PARTICLE(int(ion.size())+1,diameter,valency,valency*1.0,1.0,ein,posvec));			// copy the salt ion to the stack of all ions
...
void INTERFACE::put_saltions_outside(vector<PARTICLE>& saltion_out, int valency, double concentration, double diameter, vector<PARTICLE>& ion)
{
  ...
  unsigned int total_saltions_outside = int(2 * (concentration * 0.6) * (volume_box * unitlength * unitlength * unitlength));		// NOTE there is a change in the factor, i think it was wrong before
  ...
  while (saltion_out.size() != total_saltions_outside)
  {
    ...
    if (continuewhile == true)
      continue;
    saltion_out.push_back(PARTICLE(int(ion.size())+1,diameter,valency,valency*1.0,1.0,eout,posvec));			// create a salt ion

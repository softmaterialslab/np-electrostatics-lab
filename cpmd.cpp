...
void cpmd(vector<PARTICLE>& ion, vector<VERTEX>& s, INTERFACE& nanoparticle, vector<THERMOSTAT>& real_bath, vector<THERMOSTAT>& fake_bath, vector<BIN>& bin, CONTROL& fmdremote, CONTROL& cpmdremote)
{
  ...
  for (int num = 1; num <= cpmdremote.steps; num++)
  {
    ...
    {
      int dummy_percentage = ceil(percentage);
      int dummy_percentage2 = int(percentage);
      if (dummy_percentage%10 == 0 && dummy_percentage2%10 == 0)
  ...
  for (unsigned int b = 0; b < density_profile.size(); b++)
    error_bar.push_back( sqrt(1.0/density_profile_samples) * sqrt( mean_sq_density.at(b)/density_profile_samples - density_profile.at(b)*density_profile.at(b) ) );
  // 3. write results
